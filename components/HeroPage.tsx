  ...
  return (
    <div className="min-h-screen w-full">
      ...
      <div className="flex items-center justify-between uppercase font-semibold lg:px-14 lg:pt-5 px-5 py-5 text-sm opacity-60">
          ...
          <Link href={`/`} className="">2025/2026</Link>
          <Link href={`/`} className="lg:flex hidden">contact me</Link>
          <Link href={`/`} className="lg:hidden text-2xl"><span><IoIosMenu /></span></Link>
      </div>
      {/* herotext */}
      <div>
      ...
      <div className="lg:px-14 px-5">
        <div className="bg-red-200 h-[30rem] overflow-hidden">
          <Image src={HomeImg} alt={""} className="object-cover w-full h-full"  priority quality={75} />
        </div>
      </div>
      {/* herobtm */}
